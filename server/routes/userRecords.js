...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        const query1 = 'SELECT * FROM User WHERE email = ? OR username = ?';
        const [existingUser] = await pool.query(query1, [email, username]);
        ...
        const [results] = await pool.query(query2, [username, email, hash]);
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (await bcrypt.compare(password, passwordHash)) {
            ...
            console.log('Role ID:', user[0].roleId);
            const accessToken = CreateAccessToken(user[0].id, user[0].roleId);
            const refreshToken = CreateRefreshToken(user[0].id, user[0].roleId);
