...
router.post('/refresh_token', async (req, res) => {
    ...
    const query = 'SELECT id, refreshToken, roleId FROM User WHERE id = ?'
    const [rows, fields] = await pool.query(query, [payload.ID]);
    ...
    if (rows[0].refreshToken !== token) {
      console.log('Refresh token not in database. Rows: ');
      console.log(rows[0]);
      return res.send({ accessToken: '' });
    }
    // token exist, create new Refresh- and accesstoken
    const accessToken = CreateAccessToken(rows[0].id, rows[0].roleId);
    const newRefreshToken = CreateRefreshToken(rows[0].id, rows[0].roleId);
    //console.log(newRefreshToken);
    // update refreshtoken on user in db
    try {
      const refreshTokenQuery = 'UPDATE User SET refreshToken = ? WHERE id = ?';
    ...
    console.log('all good. New token: ');
    console.log(newRefreshToken)
