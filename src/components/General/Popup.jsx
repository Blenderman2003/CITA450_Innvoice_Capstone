...
const Popup = ({ isOpen, onClose, title, children, theme }) => {
  ...
  return (
    <div className={`fixed inset-0 ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-200'} bg-opacity-75 flex items-center justify-center`}>
      {/* Modal container */}
      <div className={`bg-white ${theme === 'dark' ? 'dark:bg-gray-900 dark:text-white' : 'text-black'} rounded-lg shadow-lg p-4 max-w-md w-full mx-2`}>
        {/* Modal header */}
        ...
        {/* Close button */}
        {/* <button onClick={onClose} className="bg-gray-300 text-black p-2 mx-1 rounded-lg mt-4">Close</button> */}
      </div>
    </div>
  );
}
